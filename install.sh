...
ph_post() {
  ...
  # write the installer-generated monitors.conf, if one was prepared
  if [[ -n "${MON_CONF:-}" ]]; then
    local mc="$HOME/.config/hypr/monitors.conf"
    [[ -f "$mc" ]] && run cp -a "$mc" "$mc.moonlit-bak"
    printf '%b' "$MON_CONF" >"$mc"
    log "wrote generated monitors.conf -> $mc"
  fi
...
# ══════════════════════════════════════════════════════════════════
#  MONITOR DETECTION  (foreground, before the install phases)
# ══════════════════════════════════════════════════════════════════
# The shipped monitors.conf is a universal wildcard (any output, preferred
# mode, scale 1) and Quickshell already spawns a bar on every screen, so
# multi-monitor works out of the box. This optionally writes explicit
# per-output lines so external/HiDPI displays get the scale you want.
MON_CONF=""
is_internal() { [[ "$1" =~ ^(eDP|LVDS|DSI) ]]; }

configure_monitors() {
  [[ ! -t 0 || ! -t 1 ]] && return   # need a TTY to ask about scales

  local names=()
  # prefer a live Hyprland session if we happen to be in one
  if command -v hyprctl >/dev/null && hyprctl monitors >/dev/null 2>&1; then
    mapfile -t names < <(hyprctl -j monitors 2>/dev/null | grep -oP '"name":\s*"\K[^"]+')
  fi
  # fallback: DRM sysfs — works from a bare TTY with no compositor running
  if [[ ${#names[@]} -eq 0 ]]; then
    local c st n
    for c in /sys/class/drm/card*-*; do
      ...
      n="$(basename "$c")"; n="${n#card*-}"   # card0-eDP-1 -> eDP-1
      names+=("$n")
    done
  fi

  if [[ ${#names[@]} -eq 0 ]]; then
    info "No monitors auto-detected — keeping the universal default (any output, preferred, scale 1)."
    return
  fi

  printf '\n   %sDetected monitors%s\n' "$B$LAV" "$R"
  local nm
  for nm in "${names[@]}"; do
    if is_internal "$nm"; then printf '     %s•%s %s  %s(internal)%s\n' "$MAUVE" "$R" "$nm" "$GREY" "$R"
    else                       printf '     %s•%s %s  %s(external)%s\n' "$MAUVE" "$R" "$nm" "$GREY" "$R"; fi
  done
  echo

  read -rp "$(printf 'Write an explicit monitors.conf for these (set per-display scale)? %s[y/N]%s ' "$GREY" "$R")" a
  [[ "${a,,}" == y* ]] || { info "Keeping the universal default monitor rule."; return; }

  # internal panel first, then externals left-to-right
  local ordered=() ext=()
  for nm in "${names[@]}"; do
    if is_internal "$nm"; then ordered+=("$nm"); else ext+=("$nm"); fi
  done
  ordered+=("${ext[@]}")

  local conf="# Generated by the Moonlit installer on $(date +%F)\n"
  conf+="# Format: monitor = NAME, RESOLUTION, POSITION, SCALE   (edit freely)\n"
  local first=1 scale pos
  for nm in "${ordered[@]}"; do
    read -rp "$(printf '  scale for %s%s%s [1]: ' "$MAUVE" "$nm" "$R")" scale
    scale="${scale:-1}"
    [[ "$scale" =~ ^[0-9]+(\.[0-9]+)?$ ]] || { warn "invalid scale '$scale', using 1"; scale=1; }
    if [[ $first -eq 1 ]]; then pos="auto"; first=0; else pos="auto-right"; fi
    conf+="monitor = ${nm}, preferred, ${pos}, ${scale}\n"
  done
  MON_CONF="$conf"
  ok "monitors.conf prepared for ${#ordered[@]} display(s); written during deploy."
}
